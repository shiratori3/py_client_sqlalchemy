...
FROM (
    ...
    FROM usrCWFZZYC A WITH(NOLOCK),
        (SELECT S_WORD, L_WORD = LEN(S_WORD) FROM (SELECT S_WORD = '股份支付费用') C) B
    ...
    AND (
        ...
        AND 
            (
                ...
                AND (
                        A.DATA LIKE '股_支___'
                    OR  A.DATA LIKE '股__付__'
                    OR  A.DATA LIKE '_份支___'
                    OR  A.DATA LIKE '_份_付__'
                )
            )
            OR (
                LEN(A.DATA) = L_WORD + 1
